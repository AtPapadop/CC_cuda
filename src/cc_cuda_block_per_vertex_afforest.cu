// src/cc_cuda_block_afforest.cu
//
// Block-per-vertex “fast GPU CC” (Afforest-style sampling + SV hooking + compression).
//
// Same algorithmic structure as thread/warp Afforest versions, but uses
// block-per-vertex cooperation for the finish phase row scan/hooking.
// (Sampling remains thread-per-vertex because k is tiny and block cooperation
// would be wasted. Compression is thread-parallel.)
//
// Phases:
//  1) parent[v] = v
//  2) Sampling rounds (k-out per vertex): thread-per-vertex
//  3) Compression passes
//  4) Pick “giant” root by sampling roots -> host mode
//  5) Finish: block-per-vertex cooperative scan of adjacency list; hook roots
...
// Sampling: thread-per-vertex (k is tiny, block-coop not worth it)
...
/**
 * Block-per-vertex finish kernel (grid-stride over vertices).
 *
 * Each block cooperatively scans u's adjacency list and performs hooking.
 * To reduce redundant work:
 *  - thread0 computes ru = find_root(u)
 ...
 */
__global__ static void finish_hook_block_kernel_skip_giant(
    uint32_t n,
    const uint64_t *row_ptr,
    const uint32_t *col_idx,
    uint32_t *parent,
    ...
    unsigned int *changed)
{
    // shared root for this u
    __shared__ uint32_t s_ru;

    const uint32_t tid = static_cast<uint32_t>(threadIdx.x);
    const uint32_t stride_u = static_cast<uint32_t>(gridDim.x);

    for (uint32_t u = static_cast<uint32_t>(blockIdx.x); u < n; u += stride_u)
    {
        if (tid == 0u)
            s_ru = d_find_root(parent, u);
        __syncthreads();
        ...
        for (uint64_t e = start + static_cast<uint64_t>(tid); e < end; e += static_cast<uint64_t>(blockDim.x))
        {
            ...
            // Refresh ru occasionally (cheap after compression)
...
int compute_connected_components_cuda_block_afforest(
    const CSRGraph *G,
    uint32_t *labels,
    ...
    double *out_kernel_seconds)
{
    ...
    uint32_t grid_finish; // capped grid for block-per-vertex finish
    uint32_t giant_root;
    uint32_t iters;
    ...
    {
        std::fprintf(stderr, "Invalid input to compute_connected_components_cuda_block_afforest\n");
        return 2;
    }
    ...
    // Cap finish grid to a multiple of SMs
    cudaDeviceProp prop;
    if (cc_cuda_check(cudaGetDeviceProperties(&prop, device_id), "cudaGetDeviceProperties") != 0)
        return 3;

    {
        uint32_t sm = static_cast<uint32_t>(prop.multiProcessorCount > 0 ? prop.multiProcessorCount : 1);
        uint32_t cap = sm * 8u;
        grid_finish = (n < cap) ? n : cap;
        if (grid_finish == 0u)
            grid_finish = 1u;
    }
    ...
    grid_threads = static_cast<uint32_t>(
    ...
    if (verbose)
    {
        std::fprintf(stderr, "[cc-cuda][block-afforest] n=%" PRIu32 ", m=%" PRIu64 "\n", G->n, G->m);
        std::fprintf(stderr, "[cc-cuda][block-afforest] block_size=%d, finish_grid=%" PRIu32 " (SM cap)\n",
                     block_size, grid_finish);
        std::fprintf(stderr, "[cc-cuda][block-afforest] K=%d, sample_rounds=%d, compress_passes=%d\n",
    ...
    init_parent_kernel<<<grid_threads, static_cast<uint32_t>(block_size)>>>(n, d_parent);
    ...
    // Phase A: sampling (thread-per-vertex)
    for (int r = 0; r < CC_AFFOREST_SAMPLE_ROUNDS; ++r)
    {
        ...
        sample_k_out_kernel<<<grid_threads, static_cast<uint32_t>(block_size)>>>(
        ...
        for (int i = 0; i < CC_AFFOREST_COMPRESS_PASSES; ++i)
            compress_step_kernel<<<grid_threads, static_cast<uint32_t>(block_size)>>>(n, d_parent);
    ...
    if (verbose)
        std::fprintf(stderr, "[cc-cuda][block-afforest] giant_root=%" PRIu32 "\n", giant_root);

    // Phase B: finish (block-per-vertex)
    iters = 0u;
    for (;;)
    {
        ...
        finish_hook_block_kernel_skip_giant<<<grid_finish, static_cast<uint32_t>(block_size)>>>(
            n, d_row_ptr, d_col_idx, d_parent, giant_root, d_changed);

        for (int i = 0; i < CC_AFFOREST_COMPRESS_PASSES; ++i)
            compress_step_kernel<<<grid_threads, static_cast<uint32_t>(block_size)>>>(n, d_parent);

        if (cc_cuda_check(cudaGetLastError(), "launch finish_hook_block / compress") != 0 ||
    ...
    write_labels_kernel<<<grid_threads, static_cast<uint32_t>(block_size)>>>(n, d_parent, d_labels_out);
