...
```bash
./bin/cc_cuda_test -m 2 -b 128,256 -r 5 -d 0 -o results ../CC/data/foo.mat
```
...
| `-m`, `--method` | Connected components kernel variant: <br> 0 = thread-per-vertex <br> 1 = thread-per-vertex frontier-async <br> 2 = thread-per-vertex afforest <br> 3 = warp-per-vertex <br> 4 = warp-per-vertex frontier-async <br> 5 = warp-per-vertex afforest <br> 6 = block-per-vertex <br>  7 = block-per-vertex frontier-async <br> 8 = block-per-vertex afforest (default 2). |
...
## Selecting a Kernel
`include/cc_cuda.h` exposes nine variants of connected components though the `compute_connected_components_cuda(...)` main warper function that accepts a 'CudaCCMethod' enum as input.

`cc_cuda_test` currently calls the thread-per-vertex afforest flavor as default. All kernels accept the same `CudaCCOptions` struct (device, block size, verbosity) and report iteration counts plus cumulative kernel time.
